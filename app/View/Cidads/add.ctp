<style>
	#modalBodyCidad{height: 130px; overflow: hidden; padding-top: 73px; }
	.close{color:#fff;}
	th{
...
<h2><?php echo __('Cadastro de  Cidade'); ?></h2>
	<!-- Modal -->
				...
				<table class="table-action col-md-12 table-bordered table-striped table-condensed cf" >
					<thead class="cf">
						<tr>
							...
							<th><?php echo $this->Paginator->sort('cidade', 'Nome');?></th>
							<th><?php echo $this->Paginator->sort('valor', 'Valor de Entrega');?></th>
							<th><?php echo $this->Paginator->sort('cobertura_total', 'Cobertura Total');?></th>
					...
					<tbody>
						...
						<tr class="<?php echo $disabledline; ?>">
							...
							<td data-title="Cobertura Total"><?php
							if($cidad['Cidad']['cobertura_total']==1){
									echo "Sim";
...
$('body').on('click','.editModal', function(event){
	...
	//	$('#loaderGif'+idpedido).show();
	//	$('#divActions'+idpedido).hide();
		$("#loadDivModal").load('http://'+urlInicio+'/cidads/view/'+modalid+'', function(){
			$('#modalLoaded').modal('show');
			/*$('#loaderGif'+idpedido).hide();
			$('#divActions'+idpedido).show();
			 $('#counter').countdown({
