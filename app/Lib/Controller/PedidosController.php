 ...
 */
 	public function reordenafila() {
		...
		$userid = $this->Session->read('Auth.User.id');
		$minhasFiliais = $User->getFiliais($userid);
 		$filas = $this->Pedido->find('all', array('recursive'=> -1, 'conditions' => array('AND'=> array(array('Pedido.statuspreparo' => 1), array('Pedido.filial_id'=> $minhasFiliais)))));
 		foreach ($filas as $fila) {
 			$id=$fila['Pedido']['id'];
 			$posicaofila= $this->Pedido->find('count', array('recursive' => -1,'conditions' => array('AND' => array(array('Pedido.filial_id'=> $minhasFiliais),array('Pedido.statuspreparo' => 1), array('Pedido.id <' => $id)))));
			$this->Pedido->id=$id;
			$this->Pedido->saveField('posicao_fila', $posicaofila);
 		}
	...
	public function add($codigo = null) {
      ...
      $clientes = $this->Cliente->find('all', array('recursive' => -1, 'order' => 'Cliente.nome asc', 'conditions'=> array('AND'=> array(array('Cliente.ativo' => true), array('Cliente.filial_id' => $filiasArray)))));
			...
			$this->set(compact('pagamentos','ultimopedido','codigo', 'produtos', 'clientes','entregadores','lojas'));
		}else{
			$entregadores = array();
			$produtos = array();
			$clientes = array();
